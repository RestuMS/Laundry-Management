...
<div x-data="expenseManager()">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="md:col-span-2 glass-container rounded-[24px] p-6 shadow-sm flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="text-slate-400 text-[13px] font-bold uppercase tracking-widest mb-1">Total Biaya (Bulan Ini)</h3>
            ...
            <div class="text-right">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-500 text-[12px] font-bold">
                    <span class="w-2 h-2 rounded-full bg-blue-400"></span> {{ \Carbon\Carbon::parse($month)->translatedFormat('F Y') }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="glass-container rounded-[28px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-8">
        <div class="p-6 border-b border-white/60 flex items-center justify-between">
            <h3 class="font-bold text-slate-700 text-[18px]">Riwayat Pengeluaran Operasional</h3>
            <form action="{{ route('expense.index') }}" method="GET" class="flex gap-2">
                <input type="month" name="month" value="{{ request('month', \Carbon\Carbon::now()->format('Y-m')) }}" onchange="this.form.submit()" class="bg-white/50 border border-slate-200 rounded-xl px-4 py-2 font-medium text-slate-600 outline-none text-[13px]">
            </form>
        </div>
            ...
            <table class="w-full text-left whitespace-nowrap min-w-[800px]">
                <thead>
                    <tr class="text-[13.5px] font-bold text-slate-400 uppercase tracking-widest border-b border-white/60">
                        ...
                        <th class="py-5 px-4">Deskripsi Biaya</th>
                        <th class="py-5 px-4">Nominal (Rp)</th>
                ...
                <tbody class="text-[14.5px] font-medium text-slate-600 align-middle">
                    ...
                    <tr class="border-b border-white/40 hover:bg-white/50 transition-colors group">
                        <td class="py-4 px-6 text-slate-500">
                            {{ \Carbon\Carbon::parse($exp->date)->translatedFormat('d F Y') }}
                        </td>
                        <td class="py-4 px-4">
                            <span class="font-bold text-slate-800 {{ strtolower($exp->note) == '' ? '' : 'mb-1' }} block">{{ $exp->name }}</span>
                            @if($exp->note)
                                <span class="text-[12px] text-slate-400 block break-words whitespace-normal max-w-xs"><span class="italic font-normal">"{{ $exp->note }}"</span></span>
                            @endif
                        </td>
                        <td class="py-4 px-4 font-black tracking-tight text-red-500 text-[16px]">
                            Rp {{ number_format($exp->amount, 0, ',', '.') }}
                        </td>
                        <td class="py-4 px-6 text-right">
                            <form action="{{ route('expense.destroy', $exp->id) }}" method="POST" onsubmit="return confirm('Hapus catatan pengeluaran kas ini?');">
                                @csrf @method('DELETE')
                                <button type="submit" class="w-9 h-9 rounded-xl bg-red-50 text-red-500 border border-red-100 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center ml-auto">
                    ...
                    @empty
                    <tr><td colspan="4" class="text-center py-10 text-slate-400">Belum ada catatan biaya keluar pada bulan ini.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
